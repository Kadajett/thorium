...
| Platform HTTP | Catalog list/search/detail, immutable ZIP delivery, PostgreSQL catalog metadata, idempotent exact-release Game Session creation, a PostgreSQL one-game-per-account registry, durable one-use surface admission, and a fenced generic Colyseus room | Deploying the new persistent artifact volume/import workflow, production identity, game-specific matchmaking/world authority, and recovery/load evidence |
