...
> **Developer preview:** Thorium is not a consumer release. The Android app is currently version `0.1.0-dev` (`versionCode` 1), and the APK produced for hands-on testing is debug-signed. Release signing, production account authentication, and validation on physical Thor hardware are not complete.
...
| --- | --- | --- |
| Native launcher | Compose catalog/search UI, bundled fallback, HTTPS catalog client, package download/install states, and two Activity/WebView surface host | Production account UI, settings/update UX, full recovery UX, and physical Thor behavior |
| Game packages | Strict `web-v1` manifest validation, deterministic ZIP packing, canonical deploy descriptor, and loopback dual-surface preview | Signing, publication workflow, and stable limits derived from Thor measurements |
| Platform HTTP | Catalog list/search/detail, immutable ZIP delivery, idempotent exact-release Game Session creation, a PostgreSQL one-game-per-account registry, durable one-use surface admission, a fenced generic Colyseus room, and a developer-preview deployment | Production identity, persistent catalog administration, game-specific matchmaking/world authority, and recovery/load evidence |
| Android runtime | Verified install/launch policy with pre-launch file rehashing, a one-call Game Session launcher, disjoint per-surface control leases and capabilities, deny-by-default WebView egress, lifecycle messages, and one semantic south-button mapping | A configured production Account Session adapter, complete controller mapping/remapping, and emulator/physical-device verification |
