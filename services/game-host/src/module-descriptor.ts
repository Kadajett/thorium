...
const Room = z.strictObject({
  ...
  filterBy: z.array(z.string().regex(/^[a-z][A-Za-z0-9]{0,47}$/)).max(8).default([]),
});

export const ServerModuleDescriptorSchema = z.strictObject({
  schema: z.literal(1),
  ...
  entrypointSizeBytes: z.number().int().positive().max(16 * 1_024 * 1_024),
